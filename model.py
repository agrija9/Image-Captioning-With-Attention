...
"""
- Decoder is the one from Neural Machine Translation with Attention paper
...
class CNN_Encoder(tf.keras.Model):
    # pass already computed features into a fully connecter layer
    def __init__(self, embedding_dim):
        super(CNN_Encoder, self).__init__()
        # shape after fc == (batch_size, 64, embedding_dim)
        self.fc = tf.keras.layers.Dense(embedding_dim)

        # shape after fully connected == (batch_size, 64, embedding_dim) 64 comes from 8*8 same dims as turbulent data
    def call(self, x):
        x = self.fc(x)
        x = tf.nn.relu(x)
        return x
